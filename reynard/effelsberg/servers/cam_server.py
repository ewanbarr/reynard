...
class EffCAMServer(AsyncDeviceServer):
    ...
    def setup_sensors(self):
        ...
        self._controller_status = Sensor.discrete(
            "controller-status",
...
class EffController(object):
    ...
    def stop(self):
        ...
        self.sensors.observing.unregister_listener(
            self.not_observing_status_handler)
        self.sensors.observing.unregister_listener(
            self.observing_status_handler)
    ...
    def observing_status_handler(self, rt, t, status, value):
        ...
        log.debug("Triggering observation start")
        self.sensors.observing.unregister_listener(
            self.observing_status_handler)
        try:
            ...
            self.sensors.observing.register_listener(
                self.not_observing_status_handler)
    ...
    def subscan_handler(self, rt, t, status, value):
        with (yield lock.acquire()):
            ...
            if int(value) == nsubscans:
                log.debug("Last subscan, deregistering subscan handlers")
                self.sensors.subscannum.unregister_listener(
                    self.subscan_handler)
            ...
            if not reading.value:
                ...
                self.ioloop.add_callback(
                    lambda: self.observing_status_handler(*reading))
    ...
    def configure_nodes(self):
        ...
        if not json_request.reply.reply_ok():
            msg = "Error on JSON request to status server: {0}".format(
                str(json_request.messages))
            raise Exception(msg)
        ...
        configure_respose = yield self._backend.req.configure(
            pack_dict(config_dict), status_string, timeout=30)
        if not configure_respose.reply.reply_ok():
            msg = "Error on configure request to backend: {0}".format(
                str(configure_respose.messages))
            raise Exception(msg)
    ...
    def stop_nodes(self):
        ...
        if not response.reply.reply_ok():
            raise Exception("Error on backend stop request: {0}".format(
                response.messages))
    ...
    def deconfigure_nodes(self):
        ...
        if not response.reply.reply_ok():
            raise Exception("Error on backend deconfigure request: {0}".format(
                response.messages))
    ...
    def scan_handler(self, rt, t, status, value):
        ...
        with (yield lock.acquire()):
            ...
            if nsubscans > 1:
                log.debug("Scan has {0} sub scans.".format(nsubscans))
                log.debug("Registering subscan handlers.")
            ...
            if not reading.value:
                ...
                self.ioloop.add_callback(
                    lambda: self.observing_status_handler(
                        *reading))
