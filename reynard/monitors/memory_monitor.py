...
class MemoryMonitor(Monitor):
    def __init__(self):
        ...
        for node in get_meminfo().keys():
            ...
            self._sensors[name_] = Sensor.float(
                name_, description="total memory on %s" %
                node, params=[
                    8192, 1e9], unit="MB", default=0)
            name_ = "%s_memory_avail" % node
            self._sensors[name_] = Sensor.float(
                name_, description="available memory on %s" %
                node, params=[
                    8192, 1e9], unit="MB", default=0)
    ...
    def update_values(self):
        ...
        for node in info.keys():
            ...
            self._sensors["%s_memory_size" %
                          node].set_value(info[node]["MemTotal"])
            self._sensors["%s_memory_avail" %
                          node].set_value(info[node]["MemFree"], status)
